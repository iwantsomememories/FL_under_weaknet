...
def separate_data(data, num_clients, num_classes, niid=False, balance=False, partition=None, class_per_client=None):
    ...
    # X: 记录每个client拥有的数据的 原始内容 
    # y: 记录每个client拥有的数据的 标签 
    # statistic: 记录每个client拥有的数据的标签统计信息
...
def split_data(X, y):
    ...
    # train_data, test_data都是长度为num_clients的list，其中的每个元素都是一个map，map包含原始数据以及标签。
